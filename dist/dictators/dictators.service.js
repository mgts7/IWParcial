"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.DictatorsService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const dictator_entity_1 = require("./entities/dictator.entity");
let DictatorsService = class DictatorsService {
    dictatorRepository;
    constructor(dictatorRepository) {
        this.dictatorRepository = dictatorRepository;
    }
    create(createDictatorDto) {
        const dictator = this.dictatorRepository.create(createDictatorDto);
        return this.dictatorRepository.save(dictator);
    }
    findAll() {
        return this.dictatorRepository.find();
    }
    findOne(id) {
        return this.dictatorRepository.findOne({ where: { id } });
    }
    async update(id, updateDictatorDto) {
        await this.dictatorRepository.update(id, updateDictatorDto);
        return this.findOne(id);
    }
    async remove(id) {
        const dictator = await this.findOne(id);
        if (dictator) {
            await this.dictatorRepository.remove(dictator);
            return { message: 'Dictator removed' };
        }
        return { message: 'Dictator not found' };
    }
};
exports.DictatorsService = DictatorsService;
exports.DictatorsService = DictatorsService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(dictator_entity_1.Dictator)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], DictatorsService);
//# sourceMappingURL=dictators.service.js.map